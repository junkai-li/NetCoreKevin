...
# 🧩 项目结构详解
        以下是项目的核心模块及其功能说明：
- 1.                             1. App.AuthorizationService
- 1.                             功能：授权服务，基于 IdentityServer4 搭建，支持单点登录（SSO）。
- 1.                             说明：用于统一管理用户认证和授权，是整个系统的安全中心。
- 1.                             2. Kevin.Common
- 1.                             功能：工具类库，包含常用帮助类、扩展方法、文件流操作、图片处理等。
- 1.                             说明：提供基础的通用功能，避免重复开发。
- 1.                             3. 文档
- 1.                             功能：基础使用文档。
- 1.                             说明：包含项目启动说明、配置步骤、数据库生成方法等。
- 1.                             4. InitData
- 1.                             功能：数据初始化的 SQL 脚本。
- 1.                             说明：用于初始化数据库结构和测试数据。
- 1.                             5. Share
- 1.                             功能：存放 DTO（数据传输对象）、结构类型等共享模型。
- 1.                             说明：用于跨模块的数据交换。
- 1.                             6. App.TaskQuartz
- 1.                             功能：基于 Quartz 的自动任务调度系统。
- 1.                             说明：用于定时执行后台任务，如数据同步、清理、推送等。
- 1.                             7. App.RepositorieRps
- 1.                             功能：数据库仓储层。
- 1.                             说明：提供对数据库的读写操作，通常使用 EFCore 实现。
- 1.                             8. App.Application
- 1.                             功能：应用服务层。
- 1.                             说明：处理业务逻辑，调用仓储层或领域层，是业务操作的中转站。
- 1.                             9. Kevin.Web
- 1.                             功能：Web 相关工具层，包含过滤器、中间件、全局类、服务初始化等。
- 1.                             说明：提供 Web 层的基础配置和扩展。
- 1.                             10. App.WebApi
- 1.                             功能：API 层。
- 1.                             说明：对外暴露的 RESTful API 接口，接收请求并调用应用服务。
- 1.                             11. kevin.CrawlingService
- 1.                             功能：基于 Selenium 的爬虫服务或自动化测试。
- 1.                             说明：用于模拟浏览器行为进行数据采集或测试。
- 1.                             12. Kevin.EntityFrameworkCore
- 1.                             功能：基于 EFCore 的工作单元（UnitOfWork）实现。
- 1.                             说明：提供统一的数据库事务管理。
- 1.                             13. kevin.Domain
- 1.                             功能：领域服务层。
- 1.                             说明：实现核心业务逻辑，是 DDD 的核心部分。
- 1.                             14. kevin.Cache
- 1.                             功能：基于 IDistributedCache 的缓存服务，支持多种缓存方式。
- 1.                             说明：提供缓存策略和操作接口。
- 1.                             15. kevin.Cap
- 1.                             功能：基于 CAP 的分布式事件总线。
- 1.                             说明：用于微服务之间的消息通信和事件处理。
- 1.                             16. kevin.Consul
- 1.                             功能：微服务注册与发现，使用 Consul 实现。
- 1.                             说明：支持服务的动态发现和负载均衡。
- 1.                             17. Kevin.Cors
- 1.                             功能：跨域配置模块。
- 1.                             说明：提供自定义的跨域策略支持。
- 1.                             18. kevin.DistributedLock
- 1.                             功能：分布式锁。
- 1.                             说明：用于在分布式环境中控制资源访问。
- 1.                             19. kevin.Domain.EventBus
- 1.                             功能：基于 MediatR 的领域事件实现。
- 1.                             说明：用于处理业务逻辑中的事件发布与订阅。
- 1.                             20. kevin.FileStorage
- 1.                             功能：文件存储服务，支持阿里云、腾讯云。
- 1.                             说明：提供文件上传、下载、管理等功能。
- 1.                             21. Kevin.HttpApiClients
- 1.                             功能：基于 IHttpClientFactory 的 HTTP 客户端。
- 1.                             说明：支持 HTTP/2.0，用于调用外部 API。
- 1.                             22. kevin.Ioc
- 1.                             功能：IOC 容器模块。
- 1.                             说明：提供模块化的依赖注入配置。
- 1.                             23. Kevin.log4Net
- 1.                             功能：日志系统，基于 log4Net。
- 1.                             说明：用于记录系统运行日志，便于调试和分析。
- 1.                             24. kevin.Permission
- 1.                             功能：权限服务，可根据 API 配置权限。
- 1.                             说明：支持基于 API 的细粒度权限控制。
- 1.                             25. Kevin.SignalR
- 1.                             功能：SignalR 实时通信服务。
- 1.                             说明：用于实时消息推送、在线状态同步等。
- 1.                             26. Kevin.SMS
- 1.                             功能：短信服务，支持阿里云、腾讯云。
- 1.                             说明：用于发送验证码、通知短信等。
- 1.                             27. Kevin.AI 系列
- 1.                             功能：AI 集成模块，包括 SemanticKernel、MCP 服务、OCR 验证码识别等。
- 1.                             说明：集成 AI 功能，如语义理解、验证码识别等。
- 
