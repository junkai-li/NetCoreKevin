...
## 更新摘要
**变更内容**
- 更新了分布式锁系统，添加了自动过期设置（2小时）以防止任务异常执行时锁的累积
- 增强了缓存服务，包括智能过期处理和默认值检索方法
- 新增了缓存服务的默认值获取方法和异步版本
- 更新了故障排查指南以反映新的分布式锁和缓存功能
...
- 缓存
  ...
  - **新增**：缓存服务现在包含智能过期处理和默认值检索方法，提供更好的容错能力。
- 数据库与 EF Core
...
- **新增**：智能过期处理
  - CacheService 现在包含智能过期处理机制，自动检测和处理过期的缓存项。
  - 提供了默认值检索方法 GetObjectOrDefault<T>() 和 GetObjectOrDefaultAsync<T>()，在缓存不存在或过期时返回默认值而不是抛出异常。
- 配置参数
...
+GetObjectOrDefault~T~(key) T?
+GetObjectOrDefaultAsync~T~(key) Task~T?~
}
class CacheService {
+ReadPayloadValue(raw, out expired) string?
+RemoveExpiredKey(key) void
}
CacheDataFilter_T_ --> ICacheService : "使用"
ICacheService <|.. CacheService : "实现"
```

图表来源
- [CacheDataFilter.cs:17-125](file://Kevin/Kevin.Web.Basics/Filters/CacheDataFilter.cs#L17-L125)
- [CacheService.cs:158-187](file://Kevin/kevin.Module/kevin.Cache/Service/CacheService.cs#L158-L187)
- [ICacheService.cs:60-74](file://Kevin/kevin.Module/kevin.Cache/Service/ICacheService.cs#L60-L74)
- [appsettings.json:12-15](file://App/WebApi/appsettings.json#L12-L15)
...
### 分布式锁系统增强
- **新增**：自动过期设置
  - 分布式锁系统现在配置了2小时的自动过期时间，防止任务异常执行时锁永久占用。
  - 通过 RedisDistributedSynchronizationOptionsBuilder 配置 Expiry(TimeSpan.FromHours(2))。
- 锁类型支持
  - 支持分布式锁、信号锁和读写锁三种类型，均配置相同的过期策略。
- 错误处理
  - 添加了异常捕获和错误日志记录，便于问题诊断。

```mermaid
flowchart TD
Start(["获取分布式锁"]) --> Check{"锁是否存在?"}
Check -- 否 --> Acquire["尝试获取锁"]
Acquire --> Success{"获取成功?"}
Success -- 是 --> Execute["执行业务逻辑"]
Execute --> Release["释放锁"]
Release --> End(["完成"])
Success -- 否 --> Wait["等待并重试"]
Wait --> Acquire
Check -- 是 --> Expired{"锁是否过期?"}
Expired -- 是 --> Remove["移除过期锁"]
Remove --> Acquire
Expired -- 否 --> Wait
```

图表来源
- [ServiceCollectionExtensions.cs:31-50](file://Kevin/kevin.Module/kevin.DistributedLock/ServiceCollectionExtensions.cs#L31-L50)
...
- **新增**：异步缓存操作
  - 缓存服务现在提供 GetStringAsync()、GetObjectOrDefaultAsync<T>() 等异步方法，避免阻塞线程池线程。
...
- [ICacheService.cs:53-74](file://Kevin/kevin.Module/kevin.Cache/Service/ICacheService.cs#L53-L74)
...
- [ResultFilter.cs:13-153](file://Kevin/Kevin.Web.Basics/Filters/ResultFilter.cs#L13-L153)
- [KevinDbContext.cs:83-133](file://Kevin/Kevin.EntityFrameworkCore/Database/KevinDbContext.cs#L83-L133)
- [ServiceCollectionExtensions.cs（分布式锁）:31-50](file://Kevin/kevin.Module/kevin.DistributedLock/ServiceCollectionExtensions.cs#L31-L50)
...
- 缓存策略
  ...
  - **新增**：使用 GetObjectOrDefault<T>() 方法处理可能不存在的缓存项，提高代码健壮性。
- 数据库优化
...
- **新增**：分布式锁优化
  - 利用2小时自动过期机制防止锁累积；监控锁的使用情况；合理设置业务逻辑的执行时间。

## 故障排查指南
- 缓存异常
  - 检查 ICacheService 注入是否正确；确认 Redis 连接串与权限；查看 CacheDataFilter 的 TTL 与键生成逻辑。
  - **新增**：检查智能过期处理是否正常工作；验证 GetObjectOrDefault<T>() 方法的返回值。
- 数据库连接失败
...
- **新增**：分布式锁问题
  - 检查 Redis 连接状态；确认2小时过期配置是否正确；监控锁的获取和释放情况。
  - 如果锁被永久占用，检查是否有异常任务未正确释放锁。
...
**最新更新**：分布式锁系统的自动过期机制和缓存服务的智能过期处理进一步提升了系统的稳定性和容错能力，特别是在处理异常任务时能够有效防止资源泄漏。
...
- [appsettings.json:37-45](file://App/WebApi/appsettings.json#L37-L45)
- [appsettings.json:46-50](file://App/WebApi/appsettings.json#L46-50)
- [appsettings.json:99-99](file://App/WebApi/appsettings.json#L99-L99)
- [ServiceCollectionExtensions.cs（分布式锁）:36-37](file://Kevin/kevin.Module/kevin.DistributedLock/ServiceCollectionExtensions.cs#L36-L37)