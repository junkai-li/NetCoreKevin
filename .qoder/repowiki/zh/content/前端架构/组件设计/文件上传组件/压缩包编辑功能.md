...
## 更新摘要
**所做更改**
- 更新了版本重建处理机制，确保编辑不同ZIP文件时实现干净的状态转换
- 新增file-ids-change事件发射器，改善组件在更大应用程序中的集成能力
- 增强了ZIP编辑器状态管理，防止状态污染和数据混淆
- 改进了文件ID变化的监听和通知机制
...
**最新更新**：组件现在支持版本重建处理和文件ID变化事件，确保在不同ZIP文件间切换时的状态隔离和更好的应用集成能力。
...
FE_EVENTS["事件系统<br/>file-ids-change / zip-updated"]
end
subgraph "后端"
...
- [FileUpload.vue:260](file://vue/kevin.web.vue/src/components/FileUpload.vue#L260)
- [FileUpload.vue:880-885](file://vue/kevin.web.vue/src/components/FileUpload.vue#L880-L885)
...
- **版本重建机制**：通过 `zipEditVersion` 和 `previewVersion` 计数器强制 Vue 销毁重建组件实例，确保每次编辑不同ZIP文件时从干净状态开始。
- **文件ID事件通知**：新增 `file-ids-change` 事件，当上传文件列表变化时向父组件通知当前文件ID数组，改善应用集成能力。
...
- [FileUpload.vue:505-518](file://vue/kevin.web.vue/src/components/FileUpload.vue#L505-L518)
- [FileUpload.vue:880-885](file://vue/kevin.web.vue/src/components/FileUpload.vue#L880-L885)
- [TextStreamReader.cs:12-36](file://Kevin/kevin.Module/Kevin.Common/Helper/FileHandleTools/TextStreamReader.cs#L12-L36)
...
- **版本控制**：通过递增版本号强制组件重建，确保状态隔离。
- **事件驱动**：文件列表变化时触发 `file-ids-change` 事件，通知父组件更新状态。
...
C->>S : 删除旧文件 + 上传新ZIP
S-->>C : 返回新文件信息
C->>P : emit('file-ids-change', fileIds)
C-->>U : 成功提示/刷新列表
```

图表来源
- [FileUpload.vue:564-575](file://vue/kevin.web.vue/src/components/FileUpload.vue#L564-L575)
- [FileUpload.vue:575-697](file://vue/kevin.web.vue/src/components/FileUpload.vue#L575-L697)
- [FileUpload.vue:699-760](file://vue/kevin.web.vue/src/components/FileUpload.vue#L699-L760)
- [FileUpload.vue:762-847](file://vue/kevin.web.vue/src/components/FileUpload.vue#L762-L847)
- [FileUpload.vue:880-885](file://vue/kevin.web.vue/src/components/FileUpload.vue#L880-L885)
- [FileService.cs:179-215](file://Kevin/Application/Services/FileService.cs#L179-L215)
...
### 版本重建处理机制
**新增功能**：为确保在不同ZIP文件间切换时实现干净的状态转换，组件引入了版本重建机制。

- **ZIP编辑器版本控制**：通过 `zipEditVersion` 计数器，每次打开新的ZIP文件时递增版本号，强制Vue销毁并重建整个编辑区域。
- **预览版本控制**：通过 `previewVersion` 计数器，确保每次预览不同文件时从干净状态开始。
- **状态清理**：版本重建时自动清理所有相关状态变量，包括文件树、内容映射、选中文件、编辑器实例等。
- **防止状态污染**：避免不同ZIP文件间的状态交叉污染，确保数据隔离。

```mermaid
flowchart TD
OpenZip["打开ZIP文件"] --> Increment["zipEditVersion++"]
Increment --> Destroy["销毁现有编辑器实例"]
Destroy --> CleanState["清理所有状态变量"]
CleanState --> Reset["重置编辑区DOM"]
Reset --> LoadNew["加载新ZIP文件"]
LoadNew --> Rebuild["重建文件树和内容映射"]
Rebuild --> Ready["准备就绪"]
```

图表来源
- [FileUpload.vue:564-575](file://vue/kevin.web.vue/src/components/FileUpload.vue#L564-L575)
- [FileUpload.vue:428-435](file://vue/kevin.web.vue/src/components/FileUpload.vue#L428-L435)
...
### 文件ID变化事件系统
**新增功能**：组件现在支持 `file-ids-change` 事件，当上传文件列表发生变化时向父组件通知当前的文件ID数组。

- **事件定义**：在组件的 `defineEmits` 中声明 `file-ids-change` 事件。
- **监听机制**：使用 `watch` 监听 `uploadedFiles` 数组的深度变化。
- **数据传递**：当事件触发时，传递当前所有已上传文件的ID数组。
- **应用场景**：父组件可以监听此事件来同步表单数据、验证规则或其他业务逻辑。

```mermaid
sequenceDiagram
participant FU as "FileUpload组件"
participant FS as "文件服务"
participant PC as "父组件"
FU->>FS : 上传文件
FS-->>FU : 返回文件信息
FU->>FU : 添加到uploadedFiles
FU->>PC : emit('file-ids-change', fileIds)
PC->>PC : 处理文件ID变化
```
...
UpdateUI --> EmitEvent["emit('zip-updated')"]
EmitEvent --> Done["完成"]
```
...
- **版本重建优化**：通过版本号强制组件重建，虽然会增加初始加载开销，但确保了状态隔离的正确性。
- **事件节流**：file-ids-change 事件在文件列表深度变化时触发，适合批量操作场景。
...
- **版本重建问题**：如果ZIP编辑器状态异常，检查 zipEditVersion 是否正确递增，确保组件实例被正确销毁和重建。
- **事件未触发**：如果父组件未收到 file-ids-change 事件，检查 uploadedFiles 的 watch 配置和事件监听器是否正确绑定。
...
**最新改进**：通过版本重建处理和文件ID事件系统，组件现在具有更好的状态管理和应用集成能力，能够在复杂的应用环境中可靠地工作。
...
- **版本管理**：利用版本重建机制，确保在不同ZIP文件间切换时的状态隔离，避免数据混淆。
- **事件监听**：在父组件中监听 `file-ids-change` 事件，实现文件ID变化的业务逻辑处理。
- **集成示例**：参考 KnowledgeBaseAddEdit.vue 中的用法，展示如何在实际应用中集成和使用该组件。

章节来源
- [KnowledgeBaseAddEdit.vue:70-78](file://vue/kevin.web.vue/src/components/ai/KnowledgeBaseAddEdit.vue#L70-L78)