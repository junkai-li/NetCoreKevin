...
namespace kevin.Application.Services.AI
{
    public class AIModelsService : BaseService, IAIModelsService
    {
        ...
        public async Task<dtoPageData<AIModelsDto>> GetPageData(dtoPagePar<string> dtoPage)
        {
            ...
            {
                data = data.Where(t => (t.ModelName ?? "").Contains(dtoPage.searchKey) || (t.ModelDescription ?? "").Contains(dtoPage.searchKey));
            }
        ...
        {
            var like = await aIModelsRp.Query(isDataPer: true).Where(t => t.IsDelete == false && t.Id == id).FirstOrDefaultAsync();
